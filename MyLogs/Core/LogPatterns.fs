...
let currentEncodePattern = patterns |> Map.find currentPatternLang |> fun x -> x.Encode


let allParsePatterns =
    let headPattern = patterns |> Map.find currentPatternLang
    let restPatterns = patterns |> Map.remove currentPatternLang |> Map.toList |> List.map (fun (_, x) -> x.Parse)
