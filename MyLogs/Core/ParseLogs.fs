...
let parseLogs (patterns: ParsePattern list) (logLines: string seq) =
                ...
                { log' with
                    Detail =
                        ...
                        | Some (Detail.Markdown _) ->
                            let detail =
                                detailStringBuilder |> Option.map (fun sb -> sb.ToString()) |> Option.defaultValue ""
                            let ternimator = Environment.NewLine
                            let length = ternimator.Length * 3
                            Detail.Markdown(
                                if detail.Length > length
                                   && detail.Substring(detail.Length - length) = ternimator + ternimator + ternimator then
    ...
    let (|Header|_|) line =
        ...
        |> Seq.tryPick (fun pattern ->
            ...
            if result.Success then Some(pattern, result.Groups[ 1 ].Value.Trim()) else None
        )
    ...
    for line in logLines do
        ...
        | Some _, _, Some (Detail.Markdown _) when detailStringBuilder.IsSome -> detailStringBuilder.Value.AppendLine line |> ignore
        ...
        | Some pattern, _, Some (Detail.Todo ls) ->
            ...
            | SafeString line -> detail <- ls @ [ { Content = line.Trim(); IsDone = false } ] |> Detail.Todo |> Some
            | _ -> ()
        ...
        | Some pattern, _, _ ->
            ...
            | RegPattern pattern.StatusDone _ -> changeLog (fun log -> { log with Status = Status.Done })
            ...
            | RegPattern pattern.DetailTodo _ -> detail <- Some(Detail.Todo [])
