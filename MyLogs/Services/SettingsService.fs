...
type SettingsService(windowSVC: IPlatformService) =
    let appSettingsPath = Environment.GetFolderPath Environment.SpecialFolder.LocalApplicationData
    let appSettingsFile = Path.Combine(appSettingsPath, "app.config")
    let detaultSettings = { Settings.DefaultValue() with LocalFolder = windowSVC.DefaultDataDir }

    let settings = cval (
        try File.ReadAllText appSettingsFile |> fromJson<Settings>
        with _ -> detaultSettings
    )
