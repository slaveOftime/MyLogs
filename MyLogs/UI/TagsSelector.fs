...
let tagsSelector onClose onSelect = html.inject <| fun (store: IShareStore, hook: IComponentHook, logsSvc: ILogsService, snackbar: ISnackbar) ->
    ...
    let saveTags newTags =
        transact (fun _ ->
            newTags |> Map.toList |> List.map snd
            |> logsSvc.WriteLogTags
            |> Observable.ofTask
            |> Observable.subscribe (function
    ...
    adaptiview(){
        ...
        MudDialog'(){
            ...
            DialogContent [
                MudContainer'() {
                    Styles [ style.width 400; style.maxHeight 500; style.height (length.perc 100); style.overflowHidden; style.displayFlex; style.flexDirectionColumn ]
                    childContent [
                        adaptiview(){
                            let! tags = filteredTags |> AVal.map (Map.toSeq >> Seq.map snd >> Seq.sortByDescending (fun x -> x.UsageCount))

                            div(){
                                styles [ style.height (length.perc 100); style.overflowXHidden; style.overflowYAuto; style.paddingRight 5 ]
                                childContent [
                                    for tag in tags  do
                                        let color' = if String.IsNullOrEmpty tag.Color then defaultColor else tag.Color
                                        MudChipSet'(){
                                            Styles [ style.displayFlex; style.alignItemsBaseline ] 
                                            onclick (fun _ -> updateTagUsageCount tag.Name; onSelect tag)
                                            childContent [
                                                MudChip'(){
                                                    Size Size.Small
                                                    Variant Variant.Outlined
                                                    Styles [ style.backgroundColor color'; style.color "white" ]
                        ...
                        adaptiview(){
                            ...
                            | Some tag ->
                                div(){
                                    styles [ style.displayFlex; style.alignItemsFlexEnd; style.flexDirectionRow; style.alignContentStretch ]
                                    childContent [
                                        MudTextFieldString'(){
                                            Value tag.Name
                                        ...
                                        MudIconButton'(){
                                            ...
                                            OnClick (fun _ -> editingTag.Publish None; saveTags (Map.add tag.Name tag tagsMap.Value))
                                            Styles [ style.padding(length.px 0, length.px 0, length.px 4, length.px 0) ]
                                        }
                                        spaceH1
                                        MudIconButton'(){
