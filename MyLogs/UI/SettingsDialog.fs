...
let settingsDialog = html.injectWithNoKey (fun (hook: IComponentHook, store: IShareStore, settingsSvc: ISettingsService, platformSvc: IPlatformService, snackbar: ISnackbar, dialog: IDialogService) ->
    ...
    let saveSettings close =
        ...
        |> Observable.subscribe (function
            | Error e -> snackbar.Add(e, Severity.Error) |> ignore; isLoading.Publish false
            | Ok _ -> isLoading.Publish false; close())
        |> hook.AddDispose

    hook.AddDispose settingsForm

    hook.OnFirstAfterRender.Add <| fun _ ->
        settingsSvc.GetSettings() |> settingsForm.SetValue
                    ...
                    MudContainer'() {
                        Styles [ 
                            ...
                            if not (windowSize = ExtraSmall || windowSize = Small) then
                                style.width 400; style.maxHeight 500
                        ]
                        childContent [
                            adaptiview(){
                                ...
                                MudTextFieldString'(){
                                    ...
                                    OnAdornmentClick (fun _ ->
                                        ...
                                        if String.IsNullOrEmpty path |> not then
                                            setLocalPath path
                                    )
