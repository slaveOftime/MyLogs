...
let moveLogToDateDialog (currentDate, log: Log) onClose onDone = html.inject <| fun (store: IShareStore, logsSvc: ILogsService, snackbar: ISnackbar, hook: IComponentHook) ->
    ...
    let move () =
        ...
        | Some targetDate when targetDate <> currentDate ->
            ...
            |> Observable.subscribe (function
                | Ok _ -> logsSvc.LoadLogs currentDate |> ignore; logsSvc.LoadLogs targetDate |> ignore; onDone()
