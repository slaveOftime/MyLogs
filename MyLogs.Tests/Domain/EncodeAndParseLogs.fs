...
"""
        ...
        }
        {
            ...
            UpdatedTime = Some(DateTimeOffset.Now.AddDays(2).ToString() |> DateTimeOffset.Parse)
            Tags = [ "WinTool" ]
            Schedule = Schedule.Range(TimeOnly.FromDateTime(DateTime.Now.AddDays(1)), TimeOnly.FromDateTime(DateTime.Now.AddDays(1)).AddMinutes(30))
            Status = Status.Done
        }
        {
            Log.Id = Guid.Empty
...
let ``Encode and parse logs should works`` (count: int) =
    ...
    for KeyValue (_, pattern) in patterns do
        ...
        |> fun x -> x.Split "\r\n" |> parseLogs readPatterns |> should equal logs
